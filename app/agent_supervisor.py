...
function_def = {
    "name": "route",
    "description": "Route the message to the appropriate worker",
    "parameters": {
        "title": "routeSchema",
        "title": "object",
        "properties": {
...
class AgentState(TypedDict):
    ...
    # be added to the existing messages.
    messages: Annotated[Sequence[BaseMessage], operator.add]
    # The 'next' field indicates the next worker to be assigned a task.
    next: str
...
research_agent = create_agent(llm, [tavily_tool], "You are a researcher tasked with finding information on the given topic.")
...
review_agent = create_agent(llm, [tavily_tool], """
                            ...
                            Instead, you just communicate directly about technical reviews.
                            """) # make this a RAG agent
review_node = functools.partial(agent_node, agent=review_agent, name="Reviewer")

test_agent = create_agent(llm, 
...
code_agent = create_agent(llm, 
                          [python_repl_tool], # DANGER DANGER
